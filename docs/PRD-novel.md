...
小说网站内点击链接可能触发 `window.open` 或 `target="_blank"` 打开系统浏览器。WebPagePlayer 通过 JS 注入拦截所有外部导航，确保所有页面跳转均在 webview 内完成。每次页面导航后自动重新注入拦截脚本。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 0.1.0 | 2026-05-14 | 初始占位文档 |
| 0.3.0 | 2026-05-14 | 完整实现：起点、七猫预设，模块 Tab 切换 |
| 1.0.0 | 2026-05-14 | 新增导航拦截机制说明，v1.0.0 发布版文档同步 |
